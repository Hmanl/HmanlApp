...
HiChat.prototype = {
    init: function() {//此方法初始化程序
        ...
        this.socket.on('connect', function() {
            //连接到服务器后，显示昵称输入框
            document.getElementById('info').textContent = '输入聊天室的昵称';
		...
		document.getElementById('loginBtn').addEventListener('click', function() {
		 ...
		 this.socket.on('nickExisted', function() {
		     document.getElementById('info').textContent = '改昵称已经被使用！'; //显示昵称被占用的提示
		 });
		 
		 this.socket.on('loginSuccess', function() {
		     document.title = '聊嗨 | ' + document.getElementById('nicknameInput').value;
		 ...
		 this.socket.on('system', function(nickName, userCount, type) {
		     //判断用户是连接还是离开以显示不同的信息
		     if(nickName){
		     	var msg = nickName + (type == 'login' ? ' 【加入聊嗨】' : ' 【离开聊嗨】');
		    ...
		    //检查是否有文件被选中
		     if (this.files.length != 0) {
		        //获取文件并用FileReader进行读取
		         ...
		         if (!reader) {
		             that._displayNewMsg('system', '你的浏览器不支持图片上传', 'red');
		             this.value = '';
		             return;
		         };
