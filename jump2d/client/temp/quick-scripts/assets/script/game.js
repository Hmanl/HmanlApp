...
cc._RF.push(module, '95f9aEkrHNCYacOW5xsm6+J', 'game', __filename);
// script/game.js
...
cc.Class({
    ...
    //主页动画判断
    judgeSpine: function judgeSpine() {
        if (lx.language == "zh") {
            //中文
    ...
    matchButton: function matchButton() {
        ...
        this.userInfo.gender == 1 ? this.setMatchInfo(avatar2, robotInfo, avatar, this.userInfo) : this.setMatchInfo(avatar, robotInfo, avatar2, this.userInfo);
    },
    setMatchInfo: function setMatchInfo(node, info, node2, userInfo) {
        ...
        sprite.scheduleOnce(function () {
            ...
            if (typeof sharedCanvas == 'undefined') {
                cc.loader.loadRes(info.avatar, cc.SpriteFrame, function (err, spriteFrame) {
                    if (cc.isValid(sprite2.node)) sprite2.spriteFrame = spriteFrame;
                });
        ...
        if (typeof sharedCanvas == 'undefined') {
            ...
            if (url) {
                ...
                image.onload = function () {
                    ...
                    texture.initWithElement(image);
                    texture.handleLoadedTexture();
                    if (cc.isValid(sprite.node)) sprite.spriteFrame = new cc.SpriteFrame(texture);
                };
                image.src = url;
            }
        }
    ...
    exitButton: function exitButton() {
        this.buttonGroup.active = true;
        if (cc.isValid(this.currPanel)) this.currPanel.active = false;
        this.exit.active = false;
        this.mask.active = false;
    },
    rankButton: function rankButton() {
    ...
    skinButton: function skinButton() {
        this.buttonGroup.active = false;
    ...
    addItem: function addItem() {
        ...
        if (this.userInfo.spring > 10) this.userInfo.spring = 10;
        if (this.userInfo.revive > 5) this.userInfo.revive = 5;
        this.userInfo.spring < 10 || this.userInfo.revive < 5 ? this.setTooltip(lx.i18n.t("label_text.Resurrection_1")) : this.setTooltip(lx.i18n.t("label_text.Resurrection_2"));
        this.exitButton();
    },
    ...
    craeteButton: function craeteButton() {
        var systemInfo = wx.getSystemInfoSync();
    ...
    initialization: function initialization(res) {
        var knock = this.layout.getChildByName('knock');
        if (cc.isValid(knock)) knock.destroy();
